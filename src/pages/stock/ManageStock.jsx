...
const ManageStock = () => {
    ...
    const fetchsctock=()=>{
        axios.get("http://localhost/PROJECT_OF_(PMS)/MY_Project_(HOME)/My_Real_Project_(Admin)/admin/api/stock/index_join_query/")
        .then((res)=>{
            console.log(res);
    ...
    const deleteData=(id)=>{
        ...
        if(isConfirm){
            axios.get("http://localhost/PROJECT_OF_(PMS)/MY_Project_(HOME)/My_Real_Project_(Admin)/admin/api/stock/delete/" + id)
            .then(res => {
              console.log(res);
              fetchsctock()
            })
