...
const ManageSupplier = () => {
    ...
    const fetchSupplier=()=>{
        axios.get("http://localhost/PROJECT_OF_(PMS)/MY_Project_(HOME)/My_Real_Project_(Admin)/admin/api/supplier/")
        .then((res)=>{
            console.log(res);
    ...
    const deleteData=(id)=>{
        ...
        if(isConfirm){
            axios.get("http://localhost/PROJECT_OF_(PMS)/MY_Project_(HOME)/My_Real_Project_(Admin)/admin/api/supplier/delete/" + id)
            .then(res => {
              console.log(res);
              fetchSupplier()
            })
                    ...
                    <tbody>
                       {
                        supplier?.map((data, i)=>{
                            ...
                            <tr key={i}>
                                <td>{data.id}</td>
                                <td>{data.photo && (<img src={"http://localhost/PROJECT_OF_(PMS)/MY_Project_(HOME)/My_Real_Project_(Admin)/admin/img/supplier/"+data.photo} alt="" height={50} width={50} />) }</td>
                                <td>{data.name}</td>
                                <td>{data.phone}</td>
                                <td>{data.email}</td>
                                <td>{data.address}</td>
